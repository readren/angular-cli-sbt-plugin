...
object AngularCliPlugin extends AutoPlugin {
	...
	object autoImport {
		...
		val confDirectory = settingKey[File]("Base directory for configuration files.")
	...
	override def projectSettings: Seq[Setting[_]] =
		Seq(
			...
			confDirectory := sourceDirectory.value / "conf",
			confDirectory in Test := sourceDirectory.value / "test-conf",
	...
	val overridenProjectSettings: Seq[Setting[_]] =
		Seq(
			...
			sourceDirectory in Compile := sourceDirectory.value,
			sourceDirectory in Test := sourceDirectory.value,
			...
			resourceDirectory in Compile := resourceDirectory.value,
			resourceDirectory in Test := resourceDirectory.value / "test-resources",
			
			(managedClasspath in Runtime) += distDirectory.value,
			(managedClasspath in Runtime) += confDirectory.value,
